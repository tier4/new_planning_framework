...
TrajectoryPointMetrics BaseEvaluator::calculate_trajectory_point_metrics(
  const std::shared_ptr<SynchronizedData> & sync_data) const
{
  ...
  for (size_t i = 0; i < num_points - 1; ++i) {
    const double time_diff = rclcpp::Duration(
    ...
    const double lateral_acc = (trajectory.points[i + 1].lateral_velocity_mps -
                               trajectory.points[i].lateral_velocity_mps) / time_resolution;
  ...
  for (size_t i = 0; i < num_points - 1; ++i) {
    ...
    longitudinal_accelerations[i] = (trajectory.points[i + 1].longitudinal_velocity_mps -
                                    trajectory.points[i].longitudinal_velocity_mps) / time_resolution;
  }
  if (num_points > 0) {
  ...
  for (size_t i = 0; i < num_points - 1; ++i) {
    ...
    const double jerk = (longitudinal_accelerations[i + 1] - longitudinal_accelerations[i]) / time_resolution;
    ...
    for (size_t i = 0; i < num_points; ++i) {
      ...
      for (const auto & object : sync_data->objects->objects) {
        const double ttc = autoware::trajectory_selector::utils::time_to_collision(
          trajectory.points[i], 
          rclcpp::Duration(trajectory.points[i].time_from_start),
          object);
        min_ttc = std::min(min_ttc, ttc);
      }
      metrics.ttc_values[i] = min_ttc;
    }
  }
  ...
  for (size_t i = 0; i < num_points; ++i) {
    metrics.travel_distances[i] = autoware::motion_utils::calcSignedArcLength(
      trajectory.points, 0, i);
  }
