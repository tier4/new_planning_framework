  ...
  const std::string & kinematic_state_topic)
{
  ...
  filter.topics.push_back(control_mode_topic);
  filter.topics.push_back(kinematic_state_topic);
  bag_reader.set_filter(filter);
  ...
  // Serialization
  rclcpp::Serialization<ControlModeReport> control_mode_serialization;
  rclcpp::Serialization<Odometry> odometry_serialization;
  ...
  while (bag_reader.has_next()) {
    ...
    if (bag_message->topic_name == control_mode_topic) {
      auto control_mode_msg = std::make_shared<ControlModeReport>();
      rclcpp::SerializedMessage serialized_msg(*bag_message->serialized_data);
      control_mode_serialization.deserialize_message(&serialized_msg, control_mode_msg.get());
    ...
    // Process odometry messages (buffer for speed lookup)
      ...
      auto odometry_msg = std::make_shared<Odometry>();
      rclcpp::SerializedMessage serialized_msg(*bag_message->serialized_data);
      odometry_serialization.deserialize_message(&serialized_msg, odometry_msg.get());
  ...
  const std::vector<OREvent> & events, const std::string & output_path)
{
  ...
  if (!output_file.is_open()) {
    RCLCPP_ERROR(logger_, "Failed to open output file: %s", output_path.c_str());
    return;
  }
  ...
  RCLCPP_INFO(logger_, "Saved %zu OR events to: %s", events.size(), output_path.c_str());
}
...
std::vector<OREvent> OREventExtractor::load_or_events_from_json(const std::string & json_path)
{
  ...
  if (!input_file.is_open()) {
    RCLCPP_ERROR(logger_, "Failed to open JSON file: %s", json_path.c_str());
    return events;
  }
  ...
  if (!j.contains("or_events")) {
    RCLCPP_ERROR(logger_, "JSON file missing 'or_events' field");
    return events;
  }
...
std::shared_ptr<OREventExtractor::Odometry> OREventExtractor::find_closest_odometry(
