...
struct OREvent
{
  ...
  rclcpp::Duration time_window = rclcpp::Duration(0, 0);  // Evaluation time window (symmetric around OR)
...
struct ORTrajectoryMetrics
{
  ...
  double time_relative_to_or_sec;            // prediction_time - OR_timestamp in seconds (negative = before OR)
  ...
  autoware_planning_msgs::msg::Trajectory ground_truth_trajectory;  // GT trajectory for visualization
};
