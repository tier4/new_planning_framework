...
ClosedLoopEvaluator::ClosedLoopEvaluator(rclcpp::Logger logger, std::shared_ptr<autoware::route_handler::RouteHandler> route_handler)
  ...
  rosbag2_cpp::Writer* bag_writer)
{
  ...
  // Save map and route markers once at the beginning
  if (bag_writer && route_handler_ && route_handler_->isHandlerReady()) {
    // Create and save map markers
    visualization_msgs::msg::MarkerArray map_markers;
    create_lanelet_map_markers(map_markers);
    if (!map_markers.markers.empty()) {
      bag_writer->write(map_markers, "/evaluation/map_markers", rclcpp::Time(0));
    }
    ...
    if (!route_markers.markers.empty()) {
      bag_writer->write(route_markers, "/evaluation/route_markers", rclcpp::Time(0));
    }
  }
  ...
  RCLCPP_INFO(logger_, 
    ...
    "Min TTC: %.3f s",
    summary_.num_samples,
    summary_.mean_lateral_error,
    summary_.max_lateral_error,
    summary_.min_ttc);
}
  ...
  for (size_t i = 0; i < trajectory.points.size() - 1; ++i) {
    ...
    if (l2 == 0.0) {
      ...
      const double dist = std::hypot(
        current_pose.position.x - p1.x,
        current_pose.position.y - p1.y);
    ...
    const double dist = std::hypot(
      current_pose.position.x - proj_x,
      current_pose.position.y - proj_y);
  ...
  const autoware_planning_msgs::msg::Trajectory& trajectory)
{
  ...
  if (closest_idx < trajectory.points.size() - 1) {
    ...
    if (l2 > 0.0) {
      const double t = ((current_pose.position.x - p1.x) * dx + 
                       (current_pose.position.y - p1.y) * dy) / l2;
  ...
  const double expected_dist = closest_idx > 0 ? 
    trajectory.points[closest_idx].longitudinal_velocity_mps * 
    rclcpp::Duration(trajectory.points[closest_idx].time_from_start).seconds() : 0.0;
  ...
  const autoware_perception_msgs::msg::PredictedObjects& objects)
{
  ...
  for (const auto& object : objects.objects) {
    ...
    const double relative_velocity = ego_velocity - 
      std::hypot(obj_twist.linear.x, obj_twist.linear.y);
...
void ClosedLoopEvaluator::calculate_summary()
{
  ...
  for (size_t i = 0; i < metrics_list_.size(); ++i) {
    ...
    summary_.max_acceleration = std::max(summary_.max_acceleration, std::abs(metrics.lateral_acceleration));
    summary_.max_jerk = std::max(summary_.max_jerk, std::abs(metrics.jerk));
    summary_.max_steering_angular_velocity = std::max(summary_.max_steering_angular_velocity, std::abs(metrics.steering_angular_velocity));
  ...
  summary_.mean_steering_angular_velocity = sum_steering_angular_velocity / static_cast<double>(summary_.num_samples);
  ...
  summary_.std_lateral_error = std::sqrt(
    sum_squared_lateral_error / static_cast<double>(summary_.num_samples) - mean_lateral_error * mean_lateral_error);
  summary_.std_steering_angle = std::sqrt(
    sum_squared_steering_angle / static_cast<double>(summary_.num_samples) - mean_steering_angle * mean_steering_angle);
...
nlohmann::json ClosedLoopEvaluator::get_summary_as_json() const
{
  ...
  j["oscillation_metrics"]["steering_angular_velocity"]["mean"] = summary_.mean_steering_angular_velocity;
  j["oscillation_metrics"]["steering_angular_velocity"]["max"] = summary_.max_steering_angular_velocity;
...
void ClosedLoopEvaluator::create_lanelet_map_markers(
  visualization_msgs::msg::MarkerArray& marker_array) const
{
  ...
  // Create visualization markers for all lanelets
  const auto all_lanelets = lanelet::utils::query::laneletLayer(lanelet_map_ptr);
  
  // Set color for map visualization (gray)
  ...
  // Create boundary markers with centerline
  marker_array = lanelet::visualization::laneletsBoundaryAsMarkerArray(
    all_lanelets, color, true, "map");
}

void ClosedLoopEvaluator::create_route_markers(
  visualization_msgs::msg::MarkerArray& marker_array) const
{
  if (!route_handler_ || !route_handler_->isHandlerReady()) {
    return;
  ...
  // Set color for route visualization (blue)
  std_msgs::msg::ColorRGBA route_color;
  route_color.r = 0.0;
  route_color.g = 0.0;
  route_color.b = 1.0;
  route_color.a = 0.8;
  ...
  marker_array = lanelet::visualization::laneletsBoundaryAsMarkerArray(
    preferred_lanes, route_color, true, "route");
  
  // Additionally, create highlight markers for preferred lanes
  std_msgs::msg::ColorRGBA preferred_color;
  preferred_color.r = 0.0;
  preferred_color.g = 1.0;
  preferred_color.b = 0.0;
  preferred_color.a = 1.0;
  
  auto preferred_markers = lanelet::visualization::laneletsBoundaryAsMarkerArray(
    preferred_lanes, preferred_color, true, "preferred");
  
  // Make preferred lane markers slightly larger
  for (auto& marker : preferred_markers.markers) {
    if (marker.type == visualization_msgs::msg::Marker::LINE_STRIP) {
      marker.scale.x *= 1.5;  // Make lines thicker
    }
  }
  ...
  marker_array.markers.insert(
    ...
    preferred_markers.markers.begin(),
    preferred_markers.markers.end());
}
