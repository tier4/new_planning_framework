...
// Common structure for trajectory point metrics
struct TrajectoryPointMetrics
{
  std::vector<double> lateral_accelerations;    // Lateral acceleration at each trajectory point
  std::vector<double> longitudinal_jerks;        // Longitudinal jerk at each trajectory point
  std::vector<double> ttc_values;               // Time to collision at each trajectory point
  std::vector<double> lateral_deviations;       // Lateral deviation from lane center at each point
  std::vector<double> travel_distances;         // Travel distance from start at each point
};
...
protected:
  ...
  /**
   * @brief Calculate metrics for each trajectory point
   * @param sync_data Synchronized data containing trajectory and objects
   * @return Trajectory point metrics
   */
  TrajectoryPointMetrics calculate_trajectory_point_metrics(
    const std::shared_ptr<SynchronizedData> & sync_data) const;
  
  /**
   * @brief Save trajectory point metrics to bag
   * @param metrics Trajectory point metrics to save
   * @param sync_data Synchronized data for timestamp reference
   * @param bag_writer Bag writer
   * @param normalized_timestamp Normalized timestamp to use
   * @param topic_prefix Prefix for topic names (e.g., "/closed_loop" or "/open_loop")
   */
  void save_trajectory_point_metrics_to_bag(
    const TrajectoryPointMetrics & metrics,
    const std::shared_ptr<SynchronizedData> & sync_data,
    rosbag2_cpp::Writer & bag_writer,
    const rclcpp::Time & normalized_timestamp) const;
